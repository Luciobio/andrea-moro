...
export const Eliminar: React.FC<DropdownProps> = ({
  id,
  title,
  optionList,
  placeholder,
  ...
  useEffect(() => {
    const newOptions: Option[] = optionList.map((data) => ({
      value: data.id,
      text: data.title,
      selected: false,
    }));
  ...
  const handleDeleteCategory = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`/api/products/multiples`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(selectedOptionValues),
      });
  ...
  const handleGetUpdatedCategories = async () => {
    try {
      const res: any = await fetch("/api/products?limit=10000", {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      const data = await res.json();
      return data.payload.map((category: OptionList) => ({
        value: category.id,
        text: category.title,
        selected: false,
      }));
